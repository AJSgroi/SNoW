...
my %subject_specific_stddev;
my %subject_specific_mean;
...
foreach my $key ( keys %subject_specific) {
	...
	#get stddev and mean
	my $tmp_mean = mean( @tmp_vec );
	my $tmp_stddev = stddev( @tmp_vec );

	#store stddev and mean
	$subject_specific_mean{$key} = $tmp_stddev;
	$subject_specific_stddev{$key} = $tmp_mean;
...
foreach my $key ( keys %subject_specific){
	...
	my $left_boundary = $subject_specific_mean{$key} - 3*$subject_specific_stddev{$key};
	...
	my $right_boundary = $subject_specific_mean{$key} + 3*$subject_specific_stddev{$key};
...
my $strong = "strong_variance_".$baseFile.".txt";
open STRONG, ">$strong";

my $weak = "weak_variance_".$baseFile.".txt";
open WEAK, ">$weak";

my $neutral = "neutral_variance_".$baseFile.".txt";
